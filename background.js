let createdWindowId = null; // globally define side window panel for managing/ updating
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    if (createdWindowId) {
      ...
      chrome.windows.get(createdWindowId, (win) => {
        if (chrome.runtime.lastError || !win) {
          // DNE or window has been closed-- create new one
          resizeAndCreateWindow(
            sender.tab.windowId,
            screenLeft,
            screenTop,
            mainWidth,
          ...
          if (win.state === "minimized") {
            // mnimized screen view-- re expand and render lead data
            chrome.windows.update(createdWindowId, { state: "normal" }, () => {
              // re expand
  ...
  } else if (request.type === "checkWindowState") {
    // check if window is open and send back state to content.js
    if (createdWindowId) {
      // not null
      chrome.windows.get(createdWindowId, (win) => {
        if (chrome.runtime.lastError || !win) {
          // side window panel is not open
...
function resizeAndCreateWindow(
  windowId,
  screenLeft,
  screenTop,
  mainWidth,
  ...
  // resize window and create side window panel
    ...
    function (updatedWindow) {
      if (chrome.runtime.lastError) {
        ...
        createNewWindow(
          ...
          screenTop,
          leadID
        ); // create side window panel and render lead data
      }
    }
  );
}
...
function updateWindowWithLeadData(leadID) {
  // get queried lead data and send to render
    ...
    .then((leadData) => {
      ...
      chrome.tabs.query({ windowId: createdWindowId }, function (tabs) {
        if (tabs.length > 0) {
          let newTabID = tabs[0].id;
          setTimeout(() => {
            // delay sending message to make sure panel.js script is loaded
...
function waitForContentScriptToLoad(windowId, leadID) {
  // get queried lead data and send to render
  chrome.tabs.query({ windowId: windowId }, function (tabs) {
    if (tabs.length > 0) {
      let newTabID = tabs[0].id;
      const checkContentScriptLoaded = setInterval(() => {
        // poll until the content script is loaded
        chrome.tabs.sendMessage(newTabID, { type: "ping" }, (response) => {
